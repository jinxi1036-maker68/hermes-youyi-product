...
def test_fact_gap_candidate_writeback_keeps_outbox_empty(tmp_path):
    ...
    result = boss.submit_fact_gap_candidate(
        ...
        gap_text="还不了解示例老师日常记录学生作业完成情况的习惯。",
        fact_owner_role="teacher",
        operation_id="fact-gap-candidate-1",
        suggested_question="示例老师，你平时每天什么时候记录学生作业完成情况？",
    ...
    assert rows[0]["related_objects"][-1]["suggested_question"].startswith("示例老师")
...
def test_night_loop_materializes_fact_gap_candidate_without_sending(tmp_path):
    ...
    def decision(_materials: dict) -> dict:
        return {
            ...
            "institution_understanding": "当前已知人员角色，但缺少示例老师记录作业完成情况的工作习惯。",
            "goal_progress_view": "",
            "observations": [],
            "work_item_updates": [],
            "questions_to_humans": [],
            ...
            "institution_fact_gaps": [
                {
                    "gap_key": "teacher_record_habit",
                    "gap_text": "缺少示例老师每天什么时候记录学生作业完成情况的习惯。",
                    "ask_role": "teacher",
                    "target_user_id": "teacher1",
                    "target_name": "示例老师",
                    "suggested_question": "示例老师，你一般几点记录学生作业完成情况？",
