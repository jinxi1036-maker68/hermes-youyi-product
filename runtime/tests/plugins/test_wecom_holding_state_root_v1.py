...
def test_existing_child_identity_mismatch_is_not_recursively_repaired(
    ...
    original_stat = child.stat()
    monkeypatch.setattr(
        gate,
        "_identity",
        lambda _user, _group: (uid + 1000, gid + 1000, {gid + 1000}),
    )
    monkeypatch.setattr(gate.os, "chown", lambda path, new_uid, new_gid: None)
    ...
    assert child.stat().st_uid == original_stat.st_uid
    assert child.stat().st_gid == original_stat.st_gid
