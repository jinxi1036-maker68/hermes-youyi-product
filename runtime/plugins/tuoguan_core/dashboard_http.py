...
<body>
  <main class="app">
    <header>
      ...
      <h1 id="title">今日运营</h1>
      <div class="sub" id="subtitle">正在读取 Hermes 预生成数据</div>
    </header>
  ...
  <script>
    ...
    function hermesTree(tree){
      ...
      return `<div class="card tree-card" style="--tree-scale:${scale};--leaf-opacity:${leafOpacity};--fruit-opacity:${fruitOpacity}">
        ...
        <div class="tree-caption"><div><div class="title">Hermes 绩效树 · Lv.${safe(tree.level || 1)}</div><div class="desc">${esc(tree.growth_text || "你和 Hermes 配合得越及时、越具体，这棵树就长得越好。")}</div></div><span class="pill">${safe(score)}分</span></div>
      </div>`;
    }
    function teacher(data, me){
      ...
      $("title").textContent = isSummerTeacher ? "2026暑假班课节记录" : "Hermes 老师成长助手";
      $("subtitle").textContent = isSummerTeacher ? safe(m.subtitle || "课节记录和孩子覆盖") : safe(companion.tone || "Hermes 是你的同事，帮你省掉重复整理和不会写的压力。");
      ...
      const render = (active="coach") => {
        ...
        const panels = {
          ...
          today: `<div class="hero"><div class="big">${esc(companion.headline || m.headline || "今天 Hermes 陪你一起把孩子情况整理清楚")}</div><div class="note">${esc((companion.helped_today||[]).join("；") || "你只要在企业微信顺手说真实情况，Hermes 会帮你整理成记录、素材和绩效证据。")}</div>${best.student_name ? `<div class="quote"><b>今天最有价值的记录</b><br>${esc(best.student_name)}：${esc(best.summary)}<div class="chips">${(best.use_cases||[]).map(x=>`<span class="chip">${esc(x)}</span>`).join("")}</div></div>` : ""}</div><section class="section"><h2>${esc(colleague.headline || "Hermes 同事一句话")}</h2><div class="card"><div class="title">${esc(colleague.message || companion.headline || "今天辛苦了，顺手说一句孩子最明显的表现就行，Hermes 帮你整理。")}</div><div class="desc">${esc(colleague.boundary_note || "未授权前不会自动私聊老师，私人情绪聊天不默认进入老板绩效材料。")}</div></div></section>${hermesTree(tree)}<div class="grid">${card("本月预估","¥"+safe(tree.estimated_amount)+" / ¥"+safe(tree.cap_amount))}${card("绩效树",safe(tree.score)+"/100")}${card("今日有效",s.today_valid_records)}${card("可用素材",safe(((data.materials||{}).quality_records||[]).length))}</div><section class="section"><h2>今天最轻松补一下</h2>${rows(companion.easy_next || data.suggested_records || [], r => `<div class="row"><div><div class="title">${esc(r.student_name || "保持今天的记录节奏")}</div><div class="desc">${esc(r.hint || r.reason || "补一句真实观察就能形成成长证据。")}</div></div><span class="pill amber">顺手补</span></div>`)}</section>`,
          ...
          materials: `<div class="hero"><div class="big">素材库是 Hermes 帮你整理出来的记录价值</div><div class="note">优质记录会自动进入这里，后续可以变成成长报告、家长沟通话术和学生标签。没有素材时，说明最近还缺少可复用的记录。</div></div><section class="section"><h2>成长报告素材</h2>${materialRows((data.materials||{}).growth_reports)}</section><section class="section"><h2>家长沟通素材</h2>${materialRows((data.materials||{}).parent_communication)}</section><section class="section"><h2>学生标签素材</h2>${materialRows((data.materials||{}).student_tags)}</section>`
        };
    ...
    function boss(data, me){
      ...
      $("title").textContent = isSummerView ? "2026暑假班视图" : (isManager ? "Hermes 店长助手" : "Hermes 数字员工工作台");
      $("subtitle").textContent = isSummerView ? "只读取 summer_2026 · 课程覆盖、证据缺口和反馈进度" : (isManager ? "Hermes 帮你盯现场、老师支持和待确认事项" : "Hermes 的在岗状态、目标推进、经营价值和需要你确认的事");
      ...
      const tabs = isManager
        ? (isSummerManager ? [{id:"assistant",label:"Hermes助手"},{id:"brief",label:"今日运行"},{id:"summer_lessons",label:"课节记录"},{id:"summer_coverage",label:"孩子覆盖"},{id:"summer_pending",label:"待处理事项"},{id:"summer_safety",label:"安全关注"},{id:"summer_feedback",label:"反馈进度"}] : [{id:"assistant",label:"Hermes助手"},{id:"brief",label:"店内状态"},{id:"exec",label:"老师支持"},{id:"closure",label:"遗留任务"}])
      ...
      const valueRows = (items) => rows(items, r => `<div class="row"><div><div class="title">${esc(r.subject || "价值推进")}</div><div class="desc">发现：${esc(r.discovered || "已记录观察")}<br>Hermes：${esc(r.hermes_action || "整理证据")}<br>结果：${esc(r.outcome || "等待真实结果")}</div></div><span class="pill">价值</span></div>`);
      const relationshipRows = (items) => rows(items, r => `<div class="row"><div><div class="title">${esc(r.message || "Hermes 主动候选")}</div><div class="desc">${esc(r.reason || "")}<br>${esc(r.value || "")}</div></div><span class="pill ${r.external_send_allowed?'':'amber'}">${esc(r.status || "candidate")}</span></div>`);
      const bossDecisionRows = (items) => {
        const list = (items || []).slice(0, 2);
        return `<div class="card decision-list">${list.length ? list.map(r => `<div class="row"><div><div class="title">${esc(short(r.question || "待确认事项", 42))}</div><div class="desc">${esc(short(r.focus_key || "Hermes 会在确认后继续推进", 54))}</div></div><span class="pill amber">${esc(r.status || "待确认")}</span></div>`).join("") : '<div class="empty">现在没有需要你立即拍板的事项</div>'}</div>`;
      };
      ...
      const bossHermesPanel = () => {
        ...
        return `<div class="boss-brief">
          <div class="boss-focus">
            <div class="kicker">Hermes 今日在岗判断</div>
          ...
          <section class="section"><h2>Hermes 主动存在感</h2>${relationshipRows((presence.items || []).slice(0,3))}</section>
      ...
      const render = (active="brief") => {
        const panels = {
          ...
          assistant: `<div class="cockpit"><div class="hero"><div class="big">${esc((assistant.headline) || "Hermes 今天帮你盯现场执行和老师支持。")}</div><div class="note">这里先给店长看最该处理的现场细节，不展示老板私密确认和全局工资。</div></div><div class="grid">${card("今日记录",safe(((assistant.store_status||{}).today_records)))}${card("7日覆盖",safe(((assistant.store_status||{}).coverage_rate_7d))+"%")}${card("未闭环",safe(((assistant.store_status||{}).open_task_count)))}${card("安全关注",safe(((assistant.store_status||{}).safety_count)))}</div></div><section class="section"><h2>Hermes 建议先看</h2>${suggestionRows(assistant.top_suggestions || [])}</section><section class="section"><h2>现场同事助手候选</h2>${relationshipRows((((assistant.relationship_support||{}).items)||[]).slice(0,3))}</section><section class="section"><h2>待确认事项</h2>${questionRows(assistant.pending_confirmations || [])}</section>${fold("assistant-goal-focus","老板目标落地",esc(((assistant.goal_focus||{}).title) || "暂无活跃目标"), workItemRows((assistant.goal_focus&&assistant.goal_focus.title)?[assistant.goal_focus]:[]),"status-blue")}`,
          ...
          business: `<div class="hero"><div class="big">经营机会</div><div class="note">先看可行动的机会和学习候选，其余细节点开处理。</div></div><section class="section"><h2>机会提醒</h2>${rows(data.opportunities, r => `<div class="row"><div><div class="title">${esc(r.name)}</div><div class="desc">${(r.sample_students||[]).map(esc).join("、")}</div></div><span class="pill">${safe(r.student_count)}人</span></div>`)}</section>${fold("boss-learning","学习动态",`待审核 ${safe(learning.pending_count)} 条。`,rows(learning.recent_candidates, r => `<div class="row"><div><div class="title">${esc(r.title || r.id)}</div><div class="desc">${esc(r.category)} · ${esc(r.candidate_type)} · ${esc(r.content)}</div></div><span class="pill amber">${esc(r.id)}</span></div>`),"status-blue")}`,
          ...
          opp: `<section class="section"><h2>经营机会</h2>${rows(data.opportunities, r => `<div class="row"><div><div class="title">${esc(r.name)}</div><div class="desc">${(r.sample_students||[]).map(esc).join("、")}</div></div><span class="pill">${safe(r.student_count)}人</span></div>`)}</section>`,
