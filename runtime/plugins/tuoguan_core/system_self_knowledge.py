...
def _query_type(compact: str) -> str:
    ...
    p4_or_test_student = any(term in compact for term in ("结业成长提升计划", "测试示例学生", "P4"))
...
def _reply(query_type: str, registry: dict[str, Any]) -> str:
    ...
    if query_type == "p4_progress":
        return "\n".join(
            [
                ...
                "- P4-16 封版验收：已通过。",
                "- P4-17 交付目录整理：已通过。",
                "- P4-8：上线前账号验证项，真实普通老师和相关老师账号尚未验证，不能写成已通过。",
                "- 当前未接生产审核，未创建真实审核任务。",
                "- 当前未推送真实普通老师或相关老师审核消息，也未生成新的业务 Word。",
    ...
    if query_type == "production_readiness":
        return (
            "现在不能直接全面启用真实老师审核。"
            "当前审核链路仍是 preview / shadow / test_state，普通老师和相关老师真实账号尚未验证；"
