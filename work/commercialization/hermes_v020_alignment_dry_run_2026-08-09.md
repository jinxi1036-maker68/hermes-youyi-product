...
本报告只做本地与资料层面的升级评估，不执行生产升级，不重启示例机构服务，不迁移数据，不触发真实外发。
...
建议新增压缩门禁测试：长会话压缩后，小优仍能接住“可以”“那就按这个来”“示例老师回复了”等短回复。

## 不建议现在启用的能力

- 不在示例机构生产直接执行 `hermes update`。
...
不升级示例机构生产 Hermes 主程序。当前只把小优业务代码按最小同步策略上线，生产升级另开计划。
