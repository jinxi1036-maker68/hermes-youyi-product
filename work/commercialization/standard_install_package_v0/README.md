...
当前阶段只做设计，不执行部署，不修改示例机构生产目录，不生成真实租户。目标是让后续 `tenant_initializer` 脚本有清晰依据，避免把示例机构数据、记忆、企业微信配置或聊天历史复制到新机构。
...
该命令只在本地 `work/commercialization/demo_output/` 生成模拟租户目录，不连接服务器、不启动 Hermes、不读取示例机构生产数据。
