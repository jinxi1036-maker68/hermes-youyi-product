...
目标：识别当前 Hermes 中哪些材料是优益样板资产，未来不能直接复制给其他机构。
...
- 优益学生名单、历史学生口径、服务关系。
- 优益老师、店长、老板企业微信映射。
- 优益当前和历史目标，例如 2026 年 9 月续费更稳。
- 优益家校沟通、学生记录、任务、工资、绩效候选。
- 优益日报、晚报、唤醒报告、价值账本。
- 优益主动提醒、回复锚定和关系经营候选。

处理方式：只能留在优益租户数据目录，不能进入模板或新机构初始包。

## 3. 当前已知写死点

这些是后续商业化前需要替换成 tenant 配置的风险点。

- 多处状态对象中写死 `tenant_id: youyi_tuoguan`。
...
- 测试中大量使用 `JinWenJie`、`金总`、`李老师`、`九月份续费率更稳` 等优益样例。
...
| 类型 | 当前证据 | 商业化处理 |
| --- | --- | --- |
| tenant_id 写死 | `runtime/plugins/tuoguan_core/digital_employee_state.py` 中多处写入 `youyi_tuoguan` | 改为从租户配置读取。 |
...
| 测试夹具 | 多个 `runtime/tests/plugins/test_youyi_*.py` 使用金总、李老师、九月续费目标 | 保留优益回归测试，同时新增通用租户测试。 |
...
- 优益企业微信密钥和身份映射。
- 优益业务错误、测试故障和临时绕路日志。
- 优益历史 H5 token、链接、访问凭证。

处理方式：只允许出现在优益生产数据和审计备份中，不进入 `tenant_profile.template.json`。
