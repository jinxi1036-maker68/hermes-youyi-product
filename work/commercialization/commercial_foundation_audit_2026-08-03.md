...
当前适合继续做“示例机构样板稳定 + 商业化地基补强”，不适合直接复制示例机构生产目录给新客户，也不建议一次性重构为复杂 SaaS 多租户。

本轮已做 P0 小修：

- 新增轻量租户上下文：运行时默认 `example_institution`，新租户可通过 `HERMES_TENANT_ID` 覆盖。
...
| 示例机构资料剥离 | 生成结果会扫描示例机构、机构负责人、示例老师、旧目标、生产路径等标记 | 基础通过 | 扩充污染词为机构 profile 驱动 |
...
- P1：`systemd/hermes-youyi-*` 仍保留示例机构生产路径和服务名，这是当前示例机构生产模板，商业化前需要做参数化服务模板。
- P1：`scripts/hermes_resource_audit.py` 仍默认示例机构生产数据目录；它是只读生产审计工具，未来应增加租户参数。
- P1：测试夹具和示例机构回归测试仍含 `example_institution`、机构负责人/示例老师等样板语境；允许保留，但需要新增通用租户回归测试。
