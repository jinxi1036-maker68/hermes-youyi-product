...
Hermes v0.20 与小优的方向高度一致，但不能直接在示例机构生产执行 `hermes update`。应先在独立临时环境验证企业微信回调插件、会话上下文、Skills、Memory、Cron、Toolsets 和生产数据目录，再另行决定升级。
...
1. Hermes 原生 `USER.md` 是一个 Hermes profile 的单一用户画像，并在会话开始时冻结注入。示例机构同一个企业微信网关服务多人，因此不能把机构负责人和示例老师偏好同时留在全局 `USER.md`。本轮提供了备份后迁移到个人工作方式档案的脚本。
